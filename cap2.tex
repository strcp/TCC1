...
abrangente, representando desde implantes de monitoramento cardíaco e \textit{transponders} para identificação animal
até automóveis com sensores integrados e sensores para análise de luminosidade e temperatura.
...
avaliação de pacientes à distância, possibilitando um atendimento mais rápido e específico, baseado em uma
...
do anúncio, os exemplos expostos pela empresa propõem um protocolo baseado em uma estrutura JSON e permitem a troca
...
curto espaço de tempo desde sua divulgação. Espera-se que seja lançado no mercado no final do ano de
2015
...
uma rede de equipamentos conectados, possibilitando o controle e a troca de informação entre eles,
...
É um sistema operacional, disponibilizado gratuitamente, desenvolvido visando a eficiência
de energia e produtividade.

A arquitetura do mbed OS fornece componentes de software e um \textit{framework} de aplicação que, combinados
...
desenvolvedores web, o que simplifica a integração entre dispositivos IoT geradores de \textit{little data},
e os servidores, que analisam e agregam a informação gerando a \textit{big data}.
...
A tecnologia \textit{FlowCloud} oferece uma plataforma segura e independente de aplicações para possibilitar
a rápida construção, gerenciamento e implantação de serviços digitais. Foi desenvolvida pela empresa
Imagination Technologies, visando suprir as necessidades dos conceitos de \textit{Internet of Things}
e \textit{Machine-to-Machine} (M2M), provendo meios de conectar dispositivos
...
é fornecida um série de ferramentas para permitir ao administrador do serviço a criação de relatórios com
...
Possui uma documentação bastante superficial de sua arquitetura, entretanto, fornece uma API implementada
...
Arrayent~\cite{arrayent} é uma plataforma que oferece conexão e segurança entre dispositivos IoT e aplicações
para  smartphones, tablets e web, com baixo custo e simplicidade. A plataforma é composta de 4 componentes:
\textit{Arrayent Connect Agent}, \textit{Arreyent Connect Cloud}, \textit{Arrayent Mobile Framework} e
\textit{Arrayent Cloud Insight}. Somente após uma série de processos burocráticos junto à empresa, a
documentação do produto pode ser obtida,assim como o kit de desenvolvimento disponível, o que torna o seu uso e o
conhecimento de seu funcionamento um pouco mais complicado comparativamente a outras alternativas como o
mbed (subseção~\ref{subsec:mbed}), ou FIWIRE (subseção~\ref{subsec:FIWARE}).

\subsubsubsection{Arrayent Connect Agent}
É utilizado por desenvolvedores de software embarcado. Funciona como um módulo do \textit{firmware} que gerencia a
seção do dispositivo junto ao Arreyent Connect Cloud e abstrai essa responsabilidade do desenvolvedor
do \textit{firmware}, deixando-o livre para se preocupar em desenvolver apenas as funcionalidades necessárias para o
...
operacional baseado na nuvem que agrega os dispositivos virtualizados, os quais são uma cópia dos dispositivos
físicos, para que as aplicações web e mobile possam se conectar e recolher informações ou executar comandos
sobre os dispositivos. Além disso, o Arreyent Connect Cloud pode emitir alertas, fazer atualização de
\textit{firmware over-the-air}, gerenciar contas de usuários e muito mais.

\subsubsubsection{Arrayent Mobile Framework}
...
O \textit{HomeKit} é um framework fornecido pela Apple para comunicação e controle de dispositivos integrantes
de uma \textit{SmartHome}. Para permitir a comunicação entre o framework e o hardware, os desenvolvedores
de hardware precisam ser compatíveis com o protocolo criado pela Apple chamado \textit{HomeKit Accessory Protocol}.
Os aplicativos utilizando o HomeKit fornecem aos usuários uma maneira simples de descobrir os acessórios compatíveis e
configurá-los. É possível também adicionar regras de negócio para estes acessórios, já que os objetos configurados
pelo HomeKit ficam armazenados em um banco de dados no dispositivo iOS do usuário, sincronizado através do iCloud
com outros dispositivos iOS que este usuário possa ter.
Apesar de ser bastante centrado nos dispositivos iOS, é possível acessar e alterar os acessórios configurados
de forma remota através do iCloud de forma segura.

Atualmente existe uma série de fabricantes que suportam o protolco HomeKit da Apple, entre elas:
\begin{itemize}
	\item \textbf{Ecobee:} Termostatos.
	\item \textbf{Elgato:} Automação de portas, janelas e avaliação de gasto energético.
	\item \textbf{iHome:} Áudio e vídeo.
	\item \textbf{Insteon:} Iluminação, termostatos, câmeras WiFi, automação de acesso, entre outros.
	\item \textbf{Lutron:} Iluminação.
\end{itemize}

A documentação é bastante rica, assim como os exemplos de código, tanto para o desenvolvimento de
software utilizando o HomeKit framework para iOS quanto para os fabricantes de hardware, entretanto,
ambos precisam de registro pago na Apple para utilização do SDK e da especificação técnica de hardware.
...
O SmartSantander irá possuir mais de 20.000 sensores para IoT distribuídos nas cidades de Belgrado,
...
\item \textbf{Nodos de IoT:}
	Responsáveis por sensorar seus respectivos parâmetros, como temperatura, luminosidade, presença de carros, etc. A maioria destes
...
tanto da sua própria arquitetura quanto o de novas aplicações para serem utilizadas no campo de testes fornecido, disponibilizando
...
Implementa funções que controlam e auditam o acesso a todas as funcionalidades do sistema. Possui processos
...
do AAA garantem que o usuário esteja propriamente autenticado e que possua a autorização necessária para
...
do ambiente de teste. As funcionalidades são geralmente utilizadas por pesquisadores e provedores de sistemas,
uma vez que um sistema pode ser visto como um experimento de longa duração.
...
O \textit{Application Support} disponibiliza funcionalidades para serem utilizadas por desenvolvedores de aplicações
e provedores de serviços, afim de compor um \textit{Smart Service} rodando nas instalações do \textit{SmartSantander}.
...
de digitalização causada pela integração das novas tecnologias relacionadas à Internet.

Seu contexto é extremamente abrangente. Seu objetivo bastante ambicioso de se tornar a Core Platform da Future Internet
torna o projeto bastante frágil, fazendo com que o fracasso de qualquer um de seus submódulos possam impactar negativamente
nos demais, além de tornar a arquitetura do projeto extremamente ampla e complexa. A questão fundamental, que é a força
motriz da iniciativa, é o fato de ser OpenSource, facilitando a participação de mais interessados no projeto e criando
novas especificações, protocolos e modelos abertos à comunidade de desenvolvedores.
Possui uma API RESTful HTTP para troca de informação sobre contexto baseada em XML~\cite{FIWARE_API}.

O projeto é baseado nos cinco módulos descritos a seguir.
...
qualificam o mercado de infraestruturas \textit{Smart}. Para possibilitar isso, foi criado o \textit{FIWARE Lab}.
